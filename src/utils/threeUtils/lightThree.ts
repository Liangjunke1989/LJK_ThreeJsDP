...
export const createAmbientLight = (
    ...
    const {color = 0xffffff, intensity = 5} = ambientLight || {};
 ...
 * 创建平行光
 * @param {string} name 光线名称
 * @param {DirectionalLight} directionalLight 构建光线参数
 * @param Position
 * @param {number} directionalLight.color  (参数可选）颜色的rgb数值。缺省值为 0xffffff
 * @param {number} directionalLight.intensity (参数可选)光照的强度。缺省值为 1
 * @returns
 */
export const createDirectionalLight = (
    ...
    const {color = 0xffffff, intensity = 1} = directionalLight || {};
